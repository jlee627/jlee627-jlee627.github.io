...
document.querySelector("#txt-search").addEventListener("keydown", txt_keydown)
...
function search()
{
  let c = document.querySelector("#txt-search").value
  ...
  if ( c != "")
  {
    ...
    {
      if (videosList[i].description.toLowerCase().indexOf(c) != -1)
      { 
        s += generateRowHTML(videosList[i])
        continue
      }
    }
  }
  ...
  if ( s == "") 
  {
    s = "No videos found with the keyword " + c
  }
...
document.querySelector("#btn-show-all").addEventListener("click", allvideos)

function allvideos()
{
  document.getElementById('results').innerHTML = ""
  
  let first = document.getElementById('results')

  for (let i = 0; i < videosList.length; i++)
    {
      first.innerHTML += generateRowHTML(videosList[i])
    }
